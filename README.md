...
# **🌐 1\. Browser Working & Rendering (1–40)**
...
# **⚡ 3\. Performance Optimization (91–120)**
...
# **🌐 3\. Node \+ Express (221–310)**

(मैं यहां पूरा 90 questions compact form में दे रहा हूँ ताकि readable रहे — numbering intact है)
...
281–310 (advanced \+ scenario):

* High traffic handle kaise?  
* Memory leak detect?  
* API slow ho toh?  
* DB bottleneck?  
* Logging strategy?  
...
# **🏗️ 5\. Remaining (361–500)**

### **System Design \+ DevOps \+ Security \+ Behavioral**
...
400. JWT security
     ...
     # **🛠️ 401–450 (REAL-WORLD EXPERIENCE QUESTIONS)**
...
450. Cross-team communication kaise handle karte ho?  
     ...
     # **🚨 451–500 (SCENARIO-BASED QUESTIONS)**
...
500. System highly available kaise banate ho?
      ...
      **💣🔥 TOP 30 TRICKY FRONTEND QUESTIONS**  
---
...
2. Difference between call, apply, bind  
3. Pollyfills  
4. Function currying  
5. Closures with many follow-up questions  
6. Event loop and its architecture  
7. Output based questions to check understanding of promise and this keyword
...
I start by understanding business requirements, then identify entities and relationships. I normalize data to reduce redundancy, define primary/foreign keys, and later optimize with indexes and selective denormalization based on query patterns 

2. What is normalization vs denormalization?  
3. What is indexing and trade-offs?  
   Indexing improves read performance by allowing fast lookups, but it increases storage and slows down writes (insert/update/delete).   
4. How do you optimize slow queries?  
   I use query profiling, check indexes, avoid full table scans, reduce joins, use pagination, and analyze execution plans   
5. What is EXPLAIN and how used?  
   In MySQL, EXPLAIN shows the query execution plan. I use it to check index usage, join type, and row scans to optimize queries.   
...
7. How do you handle large datasets?  
   Using pagination, indexing, partitioning, caching, and batching instead of loading everything at once.   
     
8. What is transaction?  
   A transaction is a group of operations executed as a single unit — either all succeed or all fail   
     
9. What is ACID?

	ACID is a set of properties that ensures **reliable and consistent database transactions**, especially in relational databases like MySQL.   
	A — Atomicity (All or Nothing) \- A transaction either completes fully or fails completely 

	C — Consistency (Valid State) \- After a transaction, the database must remain in a valid state following all rules and constraints 

	I — Isolation (No Interference)  \- Multiple transactions run independently without affecting each other.

D — Durability (Permanent Save)  \- Once a transaction is committed, it is permanently stored — even if system crashes  

10. How do you handle concurrency?  
    I use transactions, isolation levels, and locking mechanisms to avoid conflicts   
11. Optimistic vs pessimistic locking?  
12. What is N+1 problem?  
    When one query triggers multiple queries for related data, causing performance issues.  
13. How do you avoid full table scan?  
    By proper indexing, selective queries, and avoiding unnecessary conditions.   
14. What is partitioning?  
    Splitting a large table into smaller parts based on range, hash, etc., to improve performance.  
15. What is replication?  
    Copying data from primary DB to replicas for high availability and read scaling   
16. How do you handle DB scaling?  
      
17. What is indexing strategy?  
    I create indexes based on query patterns — frequently used columns in WHERE, JOIN, ORDER BY   
18. When not to use index?  
19. What is query optimization?  
    Improving query performance using indexing, rewriting queries, and reducing unnecessary operations   
20. What is connection pooling?  
    Reusing DB connections instead of creating new ones every time — improves performance   
21. How do you handle DB failures?  
    Using replication, backups, failover mechanisms, and monitoring.   
22. What is deadlock?  
    When two transactions wait on each other’s resources, causing a standstill   
23. How do you debug DB issues?  
    I check logs, slow queries, EXPLAIN plans, and monitor metrics   
24. What is data consistency?  
    Ensuring data remains accurate and valid across transactions and systems   
25. How do you archive old data?  
    Move old data to archive tables or storage to reduce load on primary DB 

# **🔥 🔥 PHP \+ LARAVEL TOP 100 QUESTIONS (8+ YEARS)**
...
# **🔥 SECTION 1: PHP (Senior Level Answers)**

### **1\. PHP versions**

In my experience, upgrading from PHP 5.6 to 7 improved performance significantly, almost 2x.  
 Later with PHP 8, we got better type safety using union types and improved error handling, which helped reduce production bugs.

---

### **2\. Memory management**

PHP handles memory automatically using garbage collection.  
 In one project, we optimized memory by using generators instead of loading large datasets, which reduced memory usage significantly.

---

### **3\. isset vs empty vs array\_key\_exists**

I usually use `isset()` for checking variables quickly.  
 When I need to check empty values like "", 0, I use `empty()`.  
 For arrays, I prefer `array_key_exists()` when null values are allowed.
...
I follow dependency injection to make code testable.  
 For example, instead of creating DB connection inside class, I inject it, which makes unit testing easier.

---

### **6\. Composer**

Composer helps manage dependencies.  
 We used PSR-4 autoloading so classes load automatically, which keeps code clean and maintainable.
...
Exceptions are used for handling expected issues like validation.  
 Errors are system-level issues. I always use try-catch for better control.
...
# **🔥 SECTION 2: Laravel (Senior Answers)**

### **11\. Lifecycle**

Request goes through middleware, routing, controller, and returns response.  
 In debugging issues, I often check middleware and service container bindings.
...
It helps manage dependencies.  
 We used it for binding interfaces to implementations, making code flexible.

---

### **14\. Eloquent vs Query Builder**

Eloquent is good for readability.  
 For heavy queries, I prefer Query Builder for performance optimization.
...
Jobs handle background tasks.  
 Events are used for decoupling logic like sending notifications.
...
For simple APIs, I prefer Sanctum.  
 For OAuth-based systems, Passport is better.
...
In one project, I chose React for flexibility.  
 For enterprise apps, Angular is better due to built-in structure.
...
Angular tracks changes automatically.  
 We optimized performance using OnPush strategy.
...
Used replication for read scaling.  
 Sharding for very large datasets.

---

# **🔥 SECTION 5: API & System Design**
